  ...
  return (
    <main className="overflow-clip bg-background text-foreground">
      ...
      <section id="reels" className="bg-chocolate py-24 text-hero-foreground lg:py-36">
        ...
        <div className="mt-14 flex touch-pan-y items-center justify-center gap-4 overflow-hidden px-[12vw]" onTouchStart={(e) => e.currentTarget.dataset.x = String(e.touches[0].clientX)} onTouchEnd={(e) => { const start = Number(e.currentTarget.dataset.x); const delta = e.changedTouches[0].clientX - start; if (Math.abs(delta) > 50) setActiveReel((activeReel + (delta < 0 ? 1 : -1) + reels.length) % reels.length); }}>
